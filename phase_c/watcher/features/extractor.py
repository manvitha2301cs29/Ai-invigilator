...
def extract_features(
    ...
    if not signals.face_present:
        ...
        phone_since, phone_flag = _update_duration_gate(
            state.phone_since, bool(signals.phone_detected), now, PHONE_SUSTAINED_SEC
        )
        second_person_since, second_person_flag = _update_duration_gate(
            state.second_person_since, bool(signals.second_person_detected), now,
        ...
        fv = FeatureVector(
            ...
            phone_flag=bool(phone_flag), second_person_flag=bool(second_person_flag),
        )
        return fv, state

    # --- Head pose classification ---
    looking_down_now = bool(abs(signals.pitch) > (
        calibration.pitch_down_exit_deg if state.was_looking_down
        else calibration.pitch_down_enter_deg
    ))
    on_screen = bool((not looking_down_now) and (abs(signals.yaw) <= calibration.yaw_envelope_deg))
    turned_now = bool((not looking_down_now) and (abs(signals.yaw) > calibration.yaw_envelope_deg))
    ...
    # --- Eyes ---
    eyes_closed_now = bool(signals.ear < calibration.ear_closed_threshold)
    ...
    fv = FeatureVector(
        ...
        eyes_closed_prolonged_flag=bool(eyes_closed_prolonged),
        phone_flag=bool(phone_flag),
        second_person_flag=bool(second_person_flag),
    )
    return fv, state
